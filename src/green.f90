...
subroutine calc_P_vertex_correction(lvertex,nm_dev,nen,nop,ndiag,dE,G_retarded,G_lesser,G_greater,W_retarded,W_lesser,W_greater,P_retarded,P_lesser,P_greater)
          ...
          do i = 1, nm_dev        
            do j = 1, nm_dev          
              if ((abs(i-k) <= ndiag).and.(abs(j-k) <= ndiag)) then                    
                do ie = max(nop+1,1),min(nen,nen+nop)
                  do iep = ie,ie                           
                    P_lesser(n,k) = P_lesser(n,k) + c1i* G_lesser(i,k,iep) * G_greater(k,j,iep-nop) &
                      * W_lesser(i,j) * G_lesser(n,i,ie) * G_greater(j,n,ie-nop) 
                    P_greater(n,k) = P_greater(n,k) + c1i* G_greater(i,k,iep) * G_lesser(k,j,iep-nop) &
                      * W_greater(i,j) * G_greater(n,i,ie) * G_lesser(j,n,ie-nop) 
                    P_retarded(n,k) = P_retarded(n,k) + &
                      c1i* G_lesser(i,k,iep) * conjg(G_retarded(j,k,iep-nop)) * W_retarded(i,j) * G_lesser(n,i,ie) * conjg(G_retarded(n,j,ie-nop)) +&
                      c1i* G_retarded(i,k,iep) * G_lesser(k,j,iep-nop) * W_retarded(i,j) * G_retarded(n,i,ie) * G_lesser(n,j,ie-nop)    
                  enddo
                enddo
              endif
            enddo
  ...
  !$omp end do
  !$omp end parallel
  P_retarded = dcmplx(dble(P_retarded),aimag(P_greater-P_lesser)/2.0d0)
...
subroutine green_solve_gw_ephoton_1D(niter,nm_dev,Lx,length,spindeg,temps,tempd,mus,mud,midgap,&
  ...
  Sig_retarded_new,Sig_lesser_new,Sig_greater_new,ldiag,encut,Egap,lvertex)
  ...
  integer::iter,ndiagmin,nop,i
    ...
    do iter=0,0
      ...
      call green_solve_gw_1D_memsaving(niter,nm_dev,Lx,length,spindeg,temps,tempd,mus,mud,midgap,&
              ...
              Sig_retarded_new(:,:,:),Sig_lesser_new(:,:,:),Sig_greater_new(:,:,:),ldiag,encut,Egap,ndiagmin=ndiagmin,lvertex=lvertex,&
    ...
    if (labs) then
      ...
      do i=1,floor(3.0d0/(En(2)-En(1)))
        PRINT *, i
        if (lvertex) then        
          call calc_pi_ephoton_exciton(spindeg,nm_dev,NB,nen,En,i,M,G_lesser,G_greater,Pi_retarded,Pi_lesser,Pi_greater,W0_retarded,W0_lesser,W0_greater)        
...
! solve BSE to calculate e-photon polarization of interacting electron-hole pair
subroutine calc_pi_ephoton_BSE(spindeg,nm_dev,ndiag,nen,En,nop,M,G_lesser,G_greater,Pi_retarded,Pi_lesser,Pi_greater,W_retarded,W_lesser,W_greater,V)
integer,intent(in)::nm_dev,nen,nop,ndiag
...
integer::ie,i,n,p,NT
integer::nn,nm,pp,pq
complex(8),allocatable::L0(:,:),LR(:,:),A(:,:),B(:,:),K(:,:) 
complex(8):: Kpn
real(8)::dE
  Pi_lesser=0.0d0
  Pi_greater=0.0d0
  Pi_retarded=0.0d0    
  dE= ( En(2) - En(1) ) / 2.0d0 / pi * spindeg
  !!
  print *, '  BSE ...'
  ! 2-point local-density approximated Bethe-Salpeter equation
  ! L_ijmn  with  i=m and j=n
  ! L_ij = G_ii * G_jj
  ! K_ij = W_ij  (only direct term, exchange term is zero under this approx)
  NT=nm_dev
  allocate(L0(NT,NT))
  allocate(LR(NT,NT))
  allocate(K (NT,NT))  
  !$omp parallel default(none) private(ie,i,n) shared(NT,nop,nen,G_lesser,G_greater,L0)
  !$omp do
  do i=1,NT
    do n=1,NT
      L0(i,n) = czero
      do ie=1,nen
        if ((ie-nop>=1).and.(ie-nop<=nen)) then
          L0(i,n) = L0(i,n) + G_greater(i,i,ie)*G_lesser(n,n,ie-nop) 
          L0(i,n) = L0(i,n) - G_lesser(i,i,ie)*G_greater(n,n,ie-nop)                             
        endif
      enddo        
    enddo
  enddo
  !$omp end do
  !$omp end parallel
  L0=dcmplx(0.0d0*dble(L0), aimag(L0)/2.0d0*dE)  
    
  !  
  Pi_retarded = L0(:,:) / (cone - L0(:,:) * W_retarded(:,:) * c1i) * (-c1i)
...
!  do i=1,nm_dev
!    do n=1,nm_dev
!      L0((i-1)*nm_dev+i,(n-1)*nm_dev+n) = Pi_retarded(i,n)
!    enddo
!  enddo
!  ! construct Kernel
!  K=czero
...
!    do n=1,NT
!      do p=1,NT
!        ! convert n,p from 2-point to 4-point index
!        nn = mod(n-1, nm_dev) + 1
!        nm = (n-1)/nm_dev + 1
!        pp = mod(p-1, nm_dev) + 1 
!        pq = (p-1)/nm_dev + 1 
!        Kpn=czero
!        if ((nn==pp).and.(nm==pq)) then
!          Kpn = Kpn + c1i* W_retarded(nn,nm)
!        endif
!        if ((nn==nm).and.(pp==pq)) then
!          Kpn = Kpn - c1i* V(nn,pp)
!        endif
!        K(i,n) = K(i,n) - L0(i,p) * Kpn
!      enddo
!    enddo
!  enddo
!  call invert(K, NT)
!  call zgemm('n','n',NT,NT,NT,cone,K,NT,L0,NT,czero,LR,NT) 
!  do i=1,nm_dev
!    do n=1,nm_dev
!      Pi_retarded(i,n)=LR((i-1)*nm_dev+i,(n-1)*nm_dev+n)
!    enddo
!  enddo    
!  deallocate(L0,LR,K)
end subroutine calc_pi_ephoton_BSE


! calculate e-photon polarization of interacting electron-hole pair with screened Coulomb interaction (static)
subroutine calc_pi_ephoton_exciton(spindeg,nm_dev,ndiag,nen,En,nop,M,G_lesser,G_greater,Pi_retarded,Pi_lesser,Pi_greater,W_retarded,W_lesser,W_greater)
integer,intent(in)::nm_dev,nen,nop,ndiag
real(8),intent(in)::en(nen),spindeg
complex(8),intent(in),dimension(nm_dev,nm_dev)::M ! e-photon interaction matrix
complex(8),intent(in),dimension(nm_dev,nm_dev,nen)::G_lesser,G_greater ! electron GF
...
complex(8)::dE
  ...
  do p = 1, nm_dev
    q=p
    do n = (p-ndiag),(p+ndiag)
      k=n
      do i = 1, nm_dev        
        j=i                
        do ie=1,nen                       
          if ((ie-nop>=1).and.(ie-nop<=nen)) then              
            do iep=max(ie-20,1),min(ie+20,nen)  
              Pi_lesser(p,q)=Pi_lesser(p,q) + &
                M(p,n)*G_lesser(i,k,ie)*G_greater(q,j,iep-nop)*W_lesser(i,j)*G_lesser(n,i,iep)*G_greater(j,p,ie-nop)*M(k,q)*(-dE*spindeg)
              Pi_greater(p,q)=Pi_greater(p,q) + &
                M(p,n)*G_greater(i,k,ie)*G_lesser(q,j,iep-nop)*W_greater(i,j)*G_greater(n,i,iep)*G_lesser(j,p,ie-nop)*M(k,q)*(-dE*spindeg)
            enddo
          endif
        enddo        
      enddo                
...
deallocate(cur,tot_cur,tot_ecur,sumtot_cur,sumtot_ecur)
!deallocate(cur_k)
deallocate(Ispec,Itot)
...
do iter=0,niter  
  ...
  ! Pij^<>(hw,kz') = \int_dE Gij^<>(E,kz) * Gji^><(E-hw,kz-kz')
  ! Pij^r(hw,kz')  = \int_dE Gij^<(E,kz) * Gji^a(E-hw,kz-kz') + Gij^r(E,kz) * Gji^<(E-hw,kz-kz')
  !$omp parallel default(none) private(l,h,iop,nop,ie,ikz,ikzd,iqz,dE,i,j,iky,ikyd,iqy,ik,iq,ikd) shared(ndiag,nopmax,P_lesser,P_greater,P_retarded,nen,En,nm_dev,G_lesser,G_greater,G_retarded,nphiz,nphiy)
  !$omp do
  do nop=-nopmax,nopmax
    do iqy=1,nphiy
      do iqz=1,nphiz
        iop=nop+nen/2
        iq=iqz+(iqy-1)*nphiz
        P_lesser(:,:,iop,iq) = dcmplx(0.0d0,0.0d0)
        P_greater(:,:,iop,iq) = dcmplx(0.0d0,0.0d0)    
          ...
          do iky=1,nphiy
            do ikz=1,nphiz              
              ...
              do i = 1, nm_dev        
                ...
                P_retarded(i,l:h,iop,iq) = P_retarded(i,l:h,iop,iq) + (G_lesser(i,l:h,ie,ik) * conjg(G_retarded(i,l:h,ie-nop,ikd)) + G_retarded(i,l:h,ie,ik) * G_lesser(l:h,i,ie-nop,ikd))        
              enddo
            enddo
          enddo
        enddo
...
!  call write_spectrum_summed_over_kz('WG',iter,W_greater, nen,En-en(nen/2),nphiy*nphiz,length,NB,Lx,(/1.0,1.0/))
  ...
  Sig_retarded_new = dcmplx(0.0d0,0.0d0)      
  ! hw from -inf to +inf: Sig^<>_ij(E) = (i/2pi) \int_dhw G^<>_ij(E-hw) W^<>_ij(hw)
  !$omp parallel default(none) private(l,h,nop,ie,i,j,iop,ikz,ikzd,iqz,iky,ikyd,iqy,ik,iq,ikd) shared(ndiag,nopmax,Sig_lesser_new,Sig_greater_new,Sig_retarded_new,W_lesser,W_greater,W_retarded,nen,En,nm_dev,G_lesser,G_greater,G_retarded,dE,nphiz,nphiy)
  !$omp do
  do ie=1,nen
   do iky=1,nphiy
    do ikz=1,nphiz
         ...
         do iqy=1,nphiy
          do iqz=1,nphiz
            ...
            do i = 1,nm_dev   
              l=max(i-ndiag,1)
              h=min(nm_dev,i+ndiag)       
              Sig_lesser_new(i,l:h,ie,ik)=Sig_lesser_new(i,l:h,ie,ik)+G_lesser(i,l:h,ie-nop,ikd)*W_lesser(i,l:h,iop,iq)
              Sig_greater_new(i,l:h,ie,ik)=Sig_greater_new(i,l:h,ie,ik)+G_greater(i,l:h,ie-nop,ikd)*W_greater(i,l:h,iop,iq)
              Sig_retarded_new(i,l:h,ie,ik)=Sig_retarded_new(i,l:h,ie,ik)+G_lesser(i,l:h,ie-nop,ikd)*W_retarded(i,l:h,iop,iq) &
                                             +G_retarded(i,l:h,ie-nop,ikd)*W_lesser(i,l:h,iop,iq) &
                                             +G_retarded(i,l:h,ie-nop,ikd)*W_retarded(i,l:h,iop,iq)          
            enddo      
          enddo
         enddo
        endif
...
subroutine green_solve_gw_1D_memsaving(niter,nm_dev,Lx,length,spindeg,temps,tempd,mus,mud,midgap,&
  alpha_mix,nen,En,nb,ns,Ham,H00lead,H10lead,T,V,&
  G_retarded,G_lesser,G_greater,Sig_retarded,Sig_lesser,Sig_greater,&
  Sig_retarded_new,Sig_lesser_new,Sig_greater_new,ldiag,encut,Egap,ndiagmin,writeGF,lvertex,W0_retarded_out,W0_lesser_out,W0_greater_out)
integer, intent(in) :: nen, nb, ns,niter,nm_dev,length
integer, intent(in), optional :: ndiagmin
real(8), intent(in) :: En(nen), temps,tempd, mus, mud, alpha_mix,Lx,spindeg, Egap,midgap(2)
complex(8),intent(in) :: Ham(nm_dev,nm_dev),H00lead(NB*NS,NB*NS,2),H10lead(NB*NS,NB*NS,2),T(NB*NS,nm_dev,2)
...
!  end if  
  ...
  do iop=1,nnop        
    ...
    if ((present(lvertex).and.lvertex).and.(iter>0)) then
      ...
      call calc_P_vertex_correction(lvertex,nm_dev,nen,nop,ndiag,(en(2)-en(1)),G_retarded,G_lesser,G_greater,W0_retarded,W0_lesser,W0_greater,P_retarded,P_lesser,P_greater)
    else
      call calc_P_vertex_correction(.false.,nm_dev,nen,nop,ndiag,(en(2)-en(1)),G_retarded,G_lesser,G_greater,W0_retarded,W0_lesser,W0_greater,P_retarded,P_lesser,P_greater)
...
!      !$omp end parallel
    ...
    P_greater=P_greater*dE  
    P_retarded=P_retarded*dE
    P_retarded=dcmplx(dble(P_retarded), 0.5d0*aimag(P_greater-P_lesser))
    if (lwriteGF) then
...
subroutine green_solve_gw_1D(niter,nm_dev,Lx,length,spindeg,temps,tempd,mus,mud,&
  ...
  !----
  complex(8),allocatable::siglead(:,:,:,:) ! lead scattering sigma_retarded
  complex(8),allocatable,dimension(:,:):: B,C,D ! tmp matrix
  ...
  !  close(101)
   ...
   ! end if  
    ...
    do i=1,nm_dev
      ...
      h=min(nm_dev,i+ndiag)
      do j=l,h
        P_lesser(i,j,:) =corr1d(nen,G_lesser(i,j,:),G_greater(j,i,:),method='sum')
        P_greater(i,j,:)=corr1d(nen,G_greater(i,j,:),G_lesser(j,i,:),method='sum')
        P_retarded(i,j,:)=corr1d(nen,G_lesser(i,j,:),conjg(G_retarded(i,j,:)),method='sum')+&
                          corr1d(nen,G_retarded(i,j,:),G_lesser(j,i,:),method='sum')
      enddo
    enddo
    !$omp end do
    !$omp end parallel
  ...
  !  call write_matrix_E('W_r',iter,W_retarded(:,:,nen/2+1:nen/2+nen),nen,en-en(nen/2),length,NB,(/1.0,1.0/))
    ...
    do i=1,nm_dev
      ...
      h=min(nm_dev,i+ndiag)
      do j=l,h
        Sig_lesser_new(i,j,:)  =conv1d(nen,G_lesser(i,j,:),W_lesser(i,j,:),method='sum')
        Sig_greater_new(i,j,:) =conv1d(nen,G_greater(i,j,:),W_greater(i,j,:),method='sum')
        Sig_retarded_new(i,j,:)=conv1d(nen,G_lesser(i,j,:),W_retarded(i,j,:),method='sum') +&
                                conv1d(nen,G_retarded(i,j,:),W_lesser(i,j,:),method='sum') +&
                                conv1d(nen,G_retarded(i,j,:),W_retarded(i,j,:),method='sum')                                
...
!
    ...
    siglead(:,:,:,2) = Sig_retarded(nm_dev-NB*NS+1:nm_dev,nm_dev-NB*NS+1:nm_dev,:)      
    !
    call write_spectrum('SigR',iter,Sig_retarded,nen,En,length,NB,Lx,(/1.0d0,1.0d0/))
    call write_spectrum('SigL',iter,Sig_lesser,nen,En,length,NB,Lx,(/1.0d0,1.0d0/))
    call write_spectrum('SigG',iter,Sig_greater,nen,En,length,NB,Lx,(/1.0d0,1.0d0/))
    !call write_matrix_summed_overE('Sigma_r',iter,Sig_retarded,nen,en,length,NB,(/1.0,1.0/))
...
subroutine homogenize_selfenergy(Sig_retarded,Sig_lesser,Sig_greater,nm_dev,nen,NB,NS)
  ...
  integer::ie
    ! make sure self-energy is continuous near leads (by copying edge block)
    do ie=1,nen
      call expand_size_bycopy(Sig_retarded(:,:,ie),nm_dev,NB,NS)
      call expand_size_bycopy(Sig_lesser(:,:,ie),nm_dev,NB,NS)
      call expand_size_bycopy(Sig_greater(:,:,ie),nm_dev,NB,NS)
    enddo
...
do ie = 1,nen
    do j = 1,length
        ...
        if (coeff(2) .ne. 0.0d0) then    
          write(11,'(4E18.4)') (j-1)*Lx, en(ie), dble(tr)*coeff(1), aimag(tr)*coeff(2)        
        else 
          write(11,'(4E18.4)') (j-1)*Lx, en(ie), dble(tr)*coeff(1)
        endif
    end do
    write(11,*)    
end do
...
subroutine sancho(nm,E,S00,H00,H10,G00,GBB)
  ...
  COMPLEX(8), ALLOCATABLE :: H_BB(:,:), H_SS(:,:), H_01(:,:), H_10(:,:)
  !
  Allocate( H_BB(nm,nm) )
  Allocate( H_SS(nm,nm) )
  Allocate( H_01(nm,nm) )
...
!        LLNN,LLNN1,LLN1N,LGNN,LGNN1,LGN1N)
    ...
    !
!    call zgemm('n','n',nm_dev,nm_dev,nm_dev,cone,WR,nm_dev,PR,nm_dev,czero,B,nm_dev)
!    call zgemm('n','c',nm_dev,nm_dev,nm_dev,cone,B,nm_dev,V,nm_dev,czero,WR,nm_dev)
    deallocate(B,M)  
  endif 
